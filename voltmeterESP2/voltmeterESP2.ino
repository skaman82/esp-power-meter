/** NON-RTC VERSION
 * ESP32 Power Meter - Battery Monitor System. Non-RTC Version 
...
struct DeviceConfig {
    ...
    float pricePerKWh = 0.305f;          // Price per kWh
};
...
void loop() {
    ...
    // Handle sensor reading every second
...
void handleEnergyLogging() {
    ...
    // Log capacity and voltage every 10 minutes (600000 ms)
    if (millis() - timing.lastCapacityMinuteMillis >= 600000UL) {
    ...
    // Log energy hourly
    if (millis() - timing.hourStartMillis >= 3600000UL) {
...
void handleMenuScreenInput() {
    if (ui.pressedButton == 2) { // Navigate menu
        ui.menustep = (ui.menustep + 1) % 9; // 9 menu items (no time setting)
